 ...
 *  1. Registers [SpotifyReceiver] dynamically (for Spotify's deprecated
 *     broadcast intents — may still work on some Spotify versions).
 *  2. Registers an internal [ActionReceiver] to receive local broadcasts
 *     from [SpotifyNotificationListener], [SpotifyReceiver], or
 *     [MainActivity] (test button).
 ...
 *
 * ## Ad detection is handled by [SpotifyNotificationListener]
 * The heavy lifting of detecting ads via MediaSession metadata is done by
 * [SpotifyNotificationListener] (a [NotificationListenerService]). It sends
 * [SpotifyReceiver.ACTION_AD_DETECTED] / [SpotifyReceiver.ACTION_MUSIC_DETECTED]
 * broadcasts that [ActionReceiver] picks up.
 *
 * ## Thread safety
 ...
 *    unregistration, volume restoration, state persistence) happens
 *    in [onDestroy].
...
class MuterService : LifecycleService() {
        ...
        when (intent?.action) {
            ACTION_START -> {
                try {
                    ...
                    Log.d(TAG, "Service started — waiting for broadcasts from SpotifyNotificationListener")
                } catch (t: Throwable) {
    ...
    /**
     * Attempts to skip the current (ad) track by dispatching
     * [KeyEvent.KEYCODE_MEDIA_NEXT] at the system level, which
     * any media app (including Spotify) responds to.
     *
     * Rate-limited by [SKIP_COOLDOWN_MS] to avoid flooding the system.
     *
